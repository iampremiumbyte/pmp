...
export async function extendLUT() {
    ...
    const keypairs1 = loadKeypairs1();
    for (const keypair of keypairs1) {
    ...
    const keypairs2 = loadKeypairs2();
    for (const keypair of keypairs2) {
        const ataToken = await spl.getAssociatedTokenAddress(
            mintKp.publicKey,
            keypair.publicKey,
        );
    ...
    const extendLUTixs5: TransactionInstruction[] = [];
    ...
    const extend5 = await buildTxn(extendLUTixs5, block1, lookupTableAccount);
