...
import { RayLiqPoolv4, connection, payer } from '../../config';
...
export async function derivePoolKeys(marketId: PublicKey) {
  ...
  const ownerBaseAta = await getOwnerAta(baseMint, payer.publicKey);
  ...
  const ownerQuoteAta = await getOwnerAta(quoteMint, payer.publicKey);
