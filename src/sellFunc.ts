import { connection, rpc, global, feeRecipient, PUMP_PROGRAM, eventAuthority, payer } from "../config";
...
export async function sellXPercentagePF() {
    ...
    const PayerTokenATA = await spl.getAssociatedTokenAddress(ca, payer.publicKey);
    ...
    for (let chunkIndex = 0; chunkIndex < chunkedKeypairs.length; chunkIndex++) {
        ...
        for (let keypair of chunk) {
          const transferAmount = await getSellBalance(keypair, ca, supplyPercent);
          ...
          const TokenATA = await spl.getAssociatedTokenAddress(ca, keypair.publicKey);
